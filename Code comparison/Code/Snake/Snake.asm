...
; Problems with game:
	...
	;6) The Color Code doesn't work when the game restarts (Ken)
	;7) The Bounderis of the game is wrong (Ken)
	;8) Asking redo for the game sometime works, but not all the time (Ken)
	;9) The speed of the snake is not the same when it moves vertically vs. horizontal (Ken)
	;10) The @ doesn't appear sometimes (Ken)
	;11) Code issue in the .data part, look at the comments (Ken)
	;12) The user might have a hard time to tell the snake head vs. tail (Ken: at least for me)
...
.data
	...
	direction 	BYTE 0  							
	olddirection 	BYTE 0								
	bricks1 	BYTE "北北北北北北北北北北北北北北北北北北北北北北北北北?",0	; missing a (") (Ken)
	bricks2 	BYTE "?					 	?",0; missing a (") (Ken)
...
main PROC
	...
	Wall:							; Wall Printing
		...
		mov edx, OFFSET bricks2		; placing it by "+"												;The comment is wrong (it is in ?)
